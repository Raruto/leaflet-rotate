L.Rotate = {
    ...
    displayPixelBounds: function() {
      var map = this._map;
      if (this.___pixelbounds) {
        this.___pixelbounds.remove();
      }

      var pixelBounds = map.getPixelBounds(),
        paddingTL = L.point(L.Icon.Default.prototype.options.iconAnchor || [0, 0]),
        paddingBR = L.point(L.Icon.Default.prototype.options.iconSize || [0, 0]).subtract(paddingTL),
        paddedBounds = L.bounds([pixelBounds.min.add(paddingTL), pixelBounds.max.subtract(paddingBR)]),
        latLngPixelBounds = L.latLngBounds(map.unproject(paddedBounds.min), map.unproject(paddedBounds.max));
      ...
      this.___pixelbounds = L.rectangle(latLngPixelBounds, {
        fill: false,
        lineCap: 'square',
        dashArray: '5, 1, 5',
        color: 'red',
