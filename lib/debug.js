L.Rotate = {
    ...
    debug: function(map, options = {}) {
        ...
        rhoInput.addEventListener('change', L.Rotate.rotate);
        rhoInput.addEventListener('mousemove', L.Rotate.rotate);
        ...
        if (options.log) {
            ...
            map.on('click', this.logEvent);

            map.on('movestart', this.logEvent);
            map.on('move', this.logEvent);
            map.on('moveend', this.logEvent);

            map.on('zoomstart', this.logEvent);
            map.on('zoomend', this.logEvent);
        }
    ...
    rotate: function(e) {
        return e.buttons !== 0 && map.setBearing(e.target.valueAsNumber);
    },
